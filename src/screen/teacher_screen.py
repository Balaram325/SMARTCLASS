...
def teacher_dashboard():
    ...
    teacher_name = teacher.get("name") or teacher.get("username") or "Teacher"
    c1, c2 = st.columns(2, vertical_alignment='center', gap='xxlarge')
    with c1:
        header_dashboard()
    with c2:
        st.markdown(f"""
            <h2 class="teacher-welcome">Welcome,<br>{teacher_name}</h2>
            <style>
                .teacher-welcome{{
...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button('Add Photos', type='primary', icon=':material/photo_prints:', width='stretch'):
            ...
            add_photos_dialog()
    # if st.session_state.show_photo_dialog:
    #     add_photos_dialog()
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter username", placeholder="Balaram Mandal")
    teacher_password = st.text_input("Enter password", type="password", placeholder="Enter password")
    st.divider()

    btc1, btc2 = st.columns(2, vertical_alignment="center", gap="xxlarge")
    with btc1:
        if st.button("Login", icon=':material/passkey:'):
            teacher = teacher_login(teacher_username.strip(), teacher_password)
            if teacher:
    ...
    with btc2:
        if st.button("Register Instead", icon=':material/passkey:', type='primary'):
...
def register_teacher(teacher_username, teacher_name, teacher_password, teacher_password_confirm):
    ...
    try:
        ...
        return False, "Unexepected Error!"
...
def teacher_screen_register():
    ...
    teacher_name = st.text_input("Enter name", placeholder="Balaram Mandal")
    teacher_username = st.text_input("Enter username", placeholder="@balarammandal")
    teacher_password = st.text_input("Enter password", type="password", placeholder="Enter password")
    ...
    with btc1:
        if st.button("Register", icon=':material/passkey:'):
            success, massage = register_teacher(teacher_username, teacher_name, teacher_password, teacher_password_confirm)
    ...
    with btc2:
        if st.button("Login Instead", icon=':material/passkey:', type='primary'):
            st.session_state.teacher_login_type = 'login'
            st.rerun()
    footer_dash()
